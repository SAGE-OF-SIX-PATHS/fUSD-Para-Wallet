...
//       // Burned fUSD (optional: listen to Burn events)
//       let burnedTokens = "0";
//       try {
//         const filter = desk.filters.Burn();
//         const events = await desk.queryFilter(filter, -5000); // last 5000 blocks
//         const totalBurned = events.reduce((acc, e) => acc + BigInt(e.args?.[1] || 0n), 0n);
//         burnedTokens = ethers.formatUnits(totalBurned, 18);
//       } catch {
//         burnedTokens = "0";
//       }
...
//       // Chart data (mock from stats for now)
//       setChartData([
//         { date: "Mon", value: Number(ethers.formatUnits(totalSupply, 18)) * 0.9 },
//         { date: "Tue", value: Number(ethers.formatUnits(totalSupply, 18)) * 0.95 },
//         { date: "Wed", value: Number(ethers.formatUnits(totalSupply, 18)) },
//         { date: "Thu", value: Number(ethers.formatUnits(totalSupply, 18)) * 1.05 },
//         { date: "Fri", value: Number(ethers.formatUnits(totalSupply, 18)) * 1.1 }
//       ]);
...
//         value: ethers.parseEther("0.01") // placeholder collateral
//       });
//       await tx.wait();
//       await fetchStats();
//     } catch (err) {
...
interface DashboardScreenProps {
  ...
  onMintTokens: (amount: string) => void;
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4">
      ...
      <div className="flex justify-between items-center mb-8">
        <div className="flex items-center space-x-2">
          <div className="flex items-center pl-10"><img src="/public/fluent.png" alt="" className="w-8 h-8 object-contain" /></div>
        </div>
        ...
        <div className="flex items-center space-x-4">
          <div className="text-right">
            <div className="text-white text-sm">
              Balance: {walletState.balanceETH || "0.00"} ETH
            </div>
            <div className="text-slate-400 text-xs">
              fUSD: {walletState.balanceFUSD || "0.00"}
            </div>
          </div>
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-700/50">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-slate-300 text-sm">TVL Collateral</span>
                <span className="text-white font-medium">
                  {protocolStats.tvlLocked || "0"}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-300 text-sm">Total fUSD Supply</span>
                <span className="text-white font-medium">
                  {protocolStats.totalSupply || "0"}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-300 text-sm">Burned fUSD</span>
                <span className="text-white font-medium">
                  {protocolStats.burnedTokens || "0"}
                </span>
              </div>
              <div className="flex items-center justify-between">
                ...
                <span className="text-green-400 font-medium">
                  {protocolStats.contractHealth || "0"}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-slate-800/50 rounded-2xl p-6 border border-slate-700/50">
            <h3 className="text-lg font-semibold text-white mb-4 flex items-center space-x-2">
              <TrendingUp className="w-5 h-5 text-green-500" /> <span>Mint fUSD</span>
            </h3>
            <input
              type="number"
              value={mintAmount}
              ...
              placeholder="Amount"
              className="w-full bg-slate-700/50 border border-slate-600 rounded-xl p-3 text-white placeholder-slate-400 focus:border-green-500"
            />
            <button
              onClick={() => onMintTokens(mintAmount)}
              disabled={!mintAmount || parseFloat(mintAmount) <= 0}
              className="mt-3 w-full bg-green-600 hover:bg-green-700 disabled:bg-slate-600 text-white font-semibold py-3 rounded-xl transition-all"
            >
              Mint Tokens
            </button>
          </div>
          ...
          <div className="bg-slate-800/50 rounded-2xl p-6 border border-slate-700/50">
            <h3 className="text-lg font-semibold text-white mb-4 flex items-center space-x-2">
              <Activity className="w-5 h-5 text-red-500" /> <span>Burn fUSD</span>
            </h3>
            <input
              type="number"
              value={burnAmount}
              ...
              placeholder="Amount"
              className="w-full bg-slate-700/50 border border-slate-600 rounded-xl p-3 text-white placeholder-slate-400 focus:border-red-500"
            />
            <button
              onClick={() => onBurnTokens(burnAmount)}
              disabled={!burnAmount || parseFloat(burnAmount) <= 0}
              className="mt-3 w-full bg-red-600 hover:bg-red-700 disabled:bg-slate-600 text-white font-semibold py-3 rounded-xl transition-all"
            >
              Burn Tokens
            </button>
          </div>
